...
function ViewJournalWrapper({ journals }: { journals: Journal[] }) {
  ...
  if (!journal) return <div style={{ color: '#fff', textAlign: 'center', marginTop: 40 }}>Journal Not Found</div>;
...
const AppRoutes: React.FC = () => {
  ...
  const [journals, setJournals] = useState<Journal[]>(() => {
    ...
    setTimeout(() => { migrateJournalsIfNeeded(); }, 0);
  ...
  async function migrateJournalsIfNeeded() {
    ...
    const migrated = await Promise.all(list.map(async (j) => {
      // 若缺 director/cast，補齊
  ...
  return (
    <Routes>
        <Route path="/" element={
          <HomePage
            journals={journals}
            onSelectJournal={journal => window.location.assign(`/journal/view/${journal.id}`)}
        ...
        <Route path="/journal/write" element={
          selectedMovie ? (
            ...
            <div style={{ color: '#fff', textAlign: 'center', marginTop: 40 }}>請先搜尋並選擇一部電影</div>
          )
        } />
        <Route path="/journal/view/:id" element={<ViewJournalWrapper journals={journals} />} />
      </Routes>
  );
};
