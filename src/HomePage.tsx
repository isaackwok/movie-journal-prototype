...
const getCoverImage = (j: Journal) => {
  if (Array.isArray(j.backdrops) && j.backdrops.length > 0)
    return j.backdrops[0];
...
const HomePage: React.FC<HomePageProps> = ({
  ...
  return (
    <div className="iphone-container">
      <div
        className="main-title special-font heading"
        style={{ color: "#fff" }}
      >
        My Movie
      </div>
      {months.map((ym) => (
        <div key={ym} style={{ marginBottom: 24 }}>
          <div
            ...
            style={{ color: "#fff", margin: "18px 0 10px 16px" }}
          >
            {getMonthLabel(ym)}
          </div>
          <div
          ...
          >
            {grouped[ym].map((j) => (
              <div
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" || e.key === " ") onSelectJournal(j);
                }}
              >
                <img
                  src={getCoverImage(j)}
                ...
                <div style={{ padding: "10px 12px 8px 12px", color: "#fff" }}>
                  <div
                    className="card-title title-font title"
                    style={{
                      whiteSpace: "nowrap",
                      overflow: "hidden",
                  ...
                  <div
                    style={{ fontSize: 13, color: "#aaa", marginBottom: 4 }}
                  ></div>
                  <div
                    style={{
                      fontSize: 14,
